---
layout: item
title: About Me!
---

Hi! I'm J. Adly (also Youssef or Joseph, depending on context).
Medical student by day, hobbyist dev by night.
I operate on the fact that life is best spent learning, building, and pushing that red button 🙄

<!-- markdownlint-disable MD033 -->
<img src="{{ site.baseurl }}/assets/img/Peter_Griffin_Do_Not_Push.png" alt="Peter Griffin DON'T Push" style="max-width: 75%;">
<!-- markdownlint-enable MD033 -->

## What Keeps Me Going

- **Building Things**: whether creating or breaking (mostly breaking), I like to
  make stuff, not just code anything (I think this is who I am)
- **Coding**: my primary building material
- **Curiosity**: my parents can tell you how annoying I was with "how" and "why"
  growing up (I still do)
- **Medicine**: yeah I'm a doctor, how awesome 🫠
- **Art & Music**: listening, sketching, and playing a bit of Lute and Piano
- **Tea**: dear Tea, please fix my life 🍵
- **Playing Brawl Stars**: the Janitor

## Education

### Minia National University

- MBBCh (Bachelor of Medicine, Bachelor of Surgery)
- Attended from 2024 to Present (Expected Graduation: 2029)

### Assiut STEM School

- High School education
- Attended from 2021 to 2024

## Work & Expertise

### Full-Stack Developer & Technical Assistant (Freelance · Remote)

- May 2025 - Oct 2025
- Developed and maintained two production REST APIs using Python/Flask with
  PostgreSQL integration for client data management
- Built a full-stack TypeScript dashboard with React frontend to visualize and
  manage API data, improving operational efficiency
- Created two Discord bots using `discord.py` for automation, moderation, and
  community engagement
- Managed a 400+ member Discord server, handling technical operations and user
  support
- Managed VPS hosting, database administration, and integrated workflows with
  Google Sheets

<!-- markdownlint-disable MD026 -->
## Let's Connect!
<!-- markdownlint-enable MD026 -->

I'm always open to collaboration or just chatting about things that spark
curiosity. Here's how you can reach me:

- **Email**: [youssefadly237@gmail.com](mailto:youssefadly237@gmail.com)
- **GitHub**: [https://github.com/youssefadly237](https://github.com/youssefadly237)
- **Twitter**: [@youssefadly237](https://x.com/youssefadly237)

Feel free to get in touch!

> Oh my God, we will keep in touch.
> Touch? The fuck! Touch each other? Where?

<!-- Source: https://www.tiktok.com/@shakemysmh/video/7461197578386803998 -->
